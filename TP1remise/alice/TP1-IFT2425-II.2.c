...
//--------------------------------------------------------------------------------
    ...
    float** visits = fmatrix_allocate_2d(length,width);
    for(i=0;i<length;i++) for(j=0;j<width;j++) visits[i][j]=0;
 //Affichage figure fractale de Mandelbrot dans Graph2D
 for(int i=0;i<length;i++) for(int j=0;j<width;j++) {

     // Converti pixels en coordonnées du plan complexe
     ...
     while (x*x+y*y<4.0 && iteration<nbIterMax) {
         float x_current =x*x-y*y+x_k;                          // partie réelle
         y=2*x*y+y_l;                                           // partie imaginaire
         x=x_current;
         iteration++;
     }
     // Points hors de l'ensemble
     if (x*x+y*y>=4.0 && iteration<nbIterMax) {
         x=0;
         y=0;
         iteration=0;
         nbIterMax=200;

         // Calcul itérations de z_n
         while (x*x+y*y<4.0 && iteration<nbIterMax) {
             float x_current = x*x-y*y+x_k;
             y = 2*x*y+y_l;
             x=x_current;

             // Converti coordonnées plan complexe en pixels
             double px_d = (x*(width-1))/ 2.0 + (double)width/1.35;
             double py_d = (y*(length-1))/ 2.0 + (double)length/2.0;

             int px= (int)(px_d + 0.5);
             int py= (int)(py_d + 0.5);

             // Vérifie coordonnés sont dans image
